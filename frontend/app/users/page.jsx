...
export default function UsersPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const connected = connRes.data.map(conn => conn.senderId === currentUser.id ? conn.receiver : conn.sender);
      setConnectedUsers(connected);
      const statuses = {};
      await Promise.all(users.map(async (u) => {
        try {
          const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/connections/status/${u.id}`, { headers: { Authorization: `Bearer ${tkn}` } });
          statuses[u.id] = res.data;
  ...
  const sendRequest = async (receiverId) => {
    ...
    } catch (err) { alert(err.response?.data?.message || 'Error'); } finally { setSendingTo(null); }
  };
  ...
  const ConnectBtn = ({ u }) => {
    ...
    if (status === 'connected') return (
      <div style={{display:'flex',flexDirection:'column',gap:'0.4rem'}}>
        ...
        <button onClick={() => disconnect(u.id)} className="btn-danger"
          style={{width:'100%',padding:'0.35rem',fontSize:'0.72rem',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.3rem'}}>
    ...
    if (status === 'sent') return (
      <div style={{display:'flex',gap:'0.3rem'}}>
        ...
        <button onClick={() => cancelRequest(u.id)} className="btn-danger" style={{padding:'0.4rem 0.6rem',fontSize:'0.72rem'}}>Cancel</button>
      </div>
    );
  ...
  const UserCard = ({ u }) => (
    <div className="glass-card pulse-hover" style={{padding:'1rem'}}>
      <div style={{display:'flex',alignItems:'center',gap:'0.65rem',marginBottom:'0.65rem'}}>
        <Link href={`/users/${u.id}`} style={{flexShrink:0}}><Avatar user={u} size={44} radius="11px"/></Link>
        <div style={{flex:1,minWidth:0}}>
          <Link href={`/users/${u.id}`} style={{fontWeight:700,color:'white',fontSize:'0.875rem',textDecoration:'none',display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
            {u.name}
          </Link>
          <p style={{color:'var(--text3)',fontSize:'0.72rem'}}>{u.department} • Batch {u.batch}</p>
          <span className={u.role==='senior'?'badge-senior':'badge-junior'} style={{marginTop:'0.15rem',display:'inline-block'}}>{u.role}</span>
        </div>
      </div>
      {u.bio && <p style={{color:'var(--text2)',fontSize:'0.75rem',marginBottom:'0.5rem',overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{u.bio}</p>}
      {u.skills && <p style={{color:'var(--text3)',fontSize:'0.7rem',marginBottom:'0.65rem'}}>🛠️ {u.skills}</p>}
      <ConnectBtn u={u}/>
    </div>
  );
    ...
    <div className="page-bg">
      ...
      <div className="main-with-sidebar">
        <div className="center-wrap" style={{flex:1,paddingTop:'2rem',paddingBottom:'3rem'}}>
          ...
          <h2 style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:'1.6rem',color:'white',marginBottom:'0.4rem'}}>Find Members</h2>
          <p style={{color:'var(--text2)',fontSize:'0.875rem',marginBottom:'1.5rem'}}>Connect with seniors, juniors, and alumni from IIUC</p>

          {/* Tabs */}
          <div style={{display:'flex',gap:'0.35rem',marginBottom:'1.25rem',flexWrap:'wrap',borderBottom:'1px solid rgba(34,197,94,0.1)',paddingBottom:'0.5rem'}}>
            {tabs.map(t => (
          ...
          {tab !== 'requests' && (
            <div style={{display:'flex',flexWrap:'wrap',gap:'0.6rem',marginBottom:'1.25rem'}}>
              ...
              <input type="text" placeholder="Department" value={department} onChange={e=>setDepartment(e.target.value)} className="input-field" style={{width:'140px'}}/>
          ...
          {tab === 'all' && (
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'0.75rem'}}>
              {filteredUsers.length === 0 ? (
                <p style={{color:'var(--text3)',gridColumn:'1/-1',textAlign:'center',padding:'2rem'}}>No members found.</p>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'0.65rem'}}>
              {requests.length === 0 ? (
                <div style={{textAlign:'center',padding:'3rem'}}>
                  ...
                  <p style={{color:'var(--text3)'}}>No pending connection requests</p>
                </div>
              ) : requests.map(req => (
                <div key={req.id} className="feed-card" style={{display:'flex',alignItems:'center',gap:'0.875rem'}}>
                  <Link href={`/users/${req.sender?.id}`} style={{flexShrink:0}}><Avatar user={req.sender} size={48} radius="12px"/></Link>
                  <div style={{flex:1,minWidth:0}}>
                    <Link href={`/users/${req.sender?.id}`} style={{fontWeight:700,color:'white',fontSize:'0.9rem',textDecoration:'none'}}>{req.sender?.name}</Link>
                    <p style={{color:'var(--text3)',fontSize:'0.75rem'}}>{req.sender?.department} • Batch {req.sender?.batch}</p>
                  ...
                  <div style={{display:'flex',gap:'0.4rem',flexShrink:0}}>
                    <button onClick={()=>acceptRequest(req.id,req.senderId)} className="btn-primary" style={{padding:'0.4rem 0.875rem',fontSize:'0.82rem',display:'flex',alignItems:'center',gap:'0.3rem'}}>
                      <UserCheck size={14}/> Accept
                    </button>
                    <button onClick={()=>declineRequest(req.id,req.senderId)} className="btn-danger" style={{padding:'0.4rem 0.875rem',fontSize:'0.82rem',display:'flex',alignItems:'center',gap:'0.3rem'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'0.75rem'}}>
              {suggestions.length === 0 ? (
                <div style={{textAlign:'center',padding:'3rem',gridColumn:'1/-1'}}>
                  ...
                  <p style={{color:'var(--text3)'}}>No suggestions found</p>
                </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'0.75rem'}}>
              {connectedList.length === 0 ? (
                <div style={{textAlign:'center',padding:'3rem',gridColumn:'1/-1'}}>
                  ...
                  <p style={{color:'var(--text3)',marginBottom:'0.5rem'}}>No connections yet</p>
                  <button onClick={()=>setTab('suggestions')} className="btn-primary" style={{padding:'0.5rem 1.25rem',fontSize:'0.85rem'}}>Find People</button>
                </div>
