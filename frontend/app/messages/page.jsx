...
// ── MessageBubble defined OUTSIDE parent to prevent re-mount on re-render
const MessageBubble = memo(({ msg, currentUser, selectedUser, reactions, onReact, onImageClick }) => {
  ...
  return (
    <div style={{
      ...
      alignItems:'flex-end', gap:'0.4rem', marginBottom:'0.4rem'
    }}>
      ...
      <div style={{maxWidth:'70%', position:'relative'}}
          ...
          <div style={{
            ...
            {MSG_EMOJIS.map(emoji => (
              <button key={emoji}
                ...
                style={{
                  background: myReaction?.emoji===emoji?'rgba(34,197,94,0.2)':'transparent',
                  border:'none', cursor:'pointer', fontSize:'1.25rem',
                  padding:'0.3rem', borderRadius:'8px', lineHeight:1
                }}>
                {emoji}
              </button>
            ))}
        ...
        <div style={{
          ...
          border: isMe?'none':'1px solid rgba(34,197,94,0.12)',
          overflow:'hidden',
        }}>
          ...
          {msg.content && (
            <div style={{padding:'0.65rem 0.9rem'}}>
              <p style={{fontSize:'0.875rem',lineHeight:'1.55',color:'white',whiteSpace:'pre-wrap',wordBreak:'break-word'}}>
                {msg.content}
              </p>
            </div>
          )}
          ...
          <div style={{display:'flex',gap:'0.2rem',flexWrap:'wrap',marginTop:'0.25rem',justifyContent:isMe?'flex-end':'flex-start'}}>
            {Object.entries(reactionCounts).map(([emoji, count]) => (
              <button key={emoji}
                ...
                style={{
                  background: myReaction?.emoji===emoji?'rgba(34,197,94,0.15)':'rgba(255,255,255,0.07)',
                  border: myReaction?.emoji===emoji?'1px solid rgba(34,197,94,0.4)':'1px solid rgba(255,255,255,0.08)',
                  borderRadius:'999px',padding:'2px 7px',fontSize:'0.73rem',cursor:'pointer',
                  display:'flex',alignItems:'center',gap:'0.2rem'
                }}>
                {emoji}
                {count > 1 && <span style={{color:'rgba(255,255,255,0.55)',fontSize:'0.68rem'}}>{count}</span>}
              </button>
            ))}
          </div>
        )}
...
export default function MessagesPage() {
  ...
  const fetchConnectedUsers = useCallback(async (tkn, currentUser) => {
    try {
      ...
      setConnectedUsers(res.data.map(conn =>
  ...
  useEffect(() => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem('mb_msg_reactions') || '{}');
      setMsgReactions(saved);
    } catch {}
    ...
    socketRef.current.on('receiveMessage', (data) => {
      ...
      if (cur && (data.senderId === cur.id || data.receiverId === cur.id)) {
    ...
    setMsgReactions(prev => {
      ...
      if (myReaction) {
        if (myReaction.emoji === emoji) {
          updated = { ...prev, [msgId]: existing.filter(r => r.userId !== myUserId) };
        } else {
          updated = { ...prev, [msgId]: existing.map(r => r.userId === myUserId ? {...r, emoji} : r) };
        }
      } else {
        updated = { ...prev, [msgId]: [...existing, { userId: myUserId, emoji }] };
      }
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/messages/send`, formData,
  ...
  const filteredContacts = [
    ...conversations.map(c => ({
      user: connectedUsers.find(u => u.id === c.user.id) || c.user,
      lastMessage: c.lastMessage, unread: c.unread
    })),
              ...
              : filteredContacts.length===0 ? (
                ...
                <div key={conv.user.id}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <p style={{color:'var(--text3)',fontSize:'0.73rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                ...
                <div style={{padding:'1rem 1.25rem',borderTop:'1px solid rgba(34,197,94,0.1)',background:'rgba(21,28,20,0.95)',flexShrink:0}}>
                  <div style={{display:'flex',gap:'0.5rem',alignItems:'center'}}>
                    ...
                    <input
                      ...
                      style={{fontSize:'0.875rem',borderRadius:'999px'}}
                      autoComplete="off"
                    />
                    <button
                      ...
                      style={{padding:'0.5rem 1rem',flexShrink:0,borderRadius:'999px'}}>
