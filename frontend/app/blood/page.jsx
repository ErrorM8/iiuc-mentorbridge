...
import {
  Droplets, Plus, X, Search, MessageSquare, Check, Trash2,
  MapPin, Calendar, AlertTriangle, Heart, Edit, Filter, Send
} from 'lucide-react';
...
const BGBadge = ({ bg, size='normal' }) => (
  <div style={{background:`${bgColor(bg)}22`,border:`1.5px solid ${bgColor(bg)}`,borderRadius:size==='large'?'12px':'8px',padding:size==='large'?'0.5rem 1rem':'0.2rem 0.6rem',display:'inline-flex',alignItems:'center',gap:'0.2rem',flexShrink:0}}>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="glass-card modal-content" style={{width:'100%',maxWidth:'420px',padding:'1.5rem'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
          <h3 style={{color:'white',fontWeight:700,fontSize:'0.95rem',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>Message Donor</h3>
          <button onClick={onClose} style={{background:'transparent',border:'none',color:'var(--text3)',cursor:'pointer'}}><X size={18}/></button>
        </div>
        ...
        {sent ? (
          <div style={{textAlign:'center',padding:'1rem',background:'rgba(34,197,94,0.08)',borderRadius:'10px',border:'1px solid rgba(34,197,94,0.2)'}}>
            ...
            <p style={{color:'var(--text3)',fontSize:'0.78rem',marginTop:'0.25rem'}}>Taking you to chat...</p>
          </div>
        ) : (
...
function FilterDropdown({ filterBG, onChange, onClose }) {
  return (
    <div style={{position:'absolute',top:'calc(100% + 6px)',left:0,background:'rgba(10,20,10,0.97)',backdropFilter:'blur(24px)',border:'1px solid rgba(34,197,94,0.2)',borderRadius:'12px',padding:'0.5rem',zIndex:50,boxShadow:'0 8px 24px rgba(0,0,0,0.5)',minWidth:'200px'}}>
      <p style={{color:'var(--text3)',fontSize:'0.7rem',fontWeight:600,padding:'0.3rem 0.65rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Blood Group</p>
      <button onClick={()=>{onChange('all');onClose();}} style={{display:'flex',alignItems:'center',gap:'0.5rem',padding:'0.45rem 0.65rem',background:filterBG==='all'?'rgba(255,255,255,0.1)':'transparent',border:'none',borderRadius:'7px',color:filterBG==='all'?'white':'var(--text2)',cursor:'pointer',width:'100%',fontSize:'0.82rem',fontWeight:filterBG==='all'?600:400}}>
        🩸 All Blood Groups
      </button>
      {BLOOD_GROUPS.map(bg => (
        <button key={bg} onClick={()=>{onChange(bg);onClose();}} style={{display:'flex',alignItems:'center',gap:'0.5rem',padding:'0.45rem 0.65rem',background:filterBG===bg?`${bgColor(bg)}22`:'transparent',border:'none',borderRadius:'7px',color:filterBG===bg?bgColor(bg):'var(--text2)',cursor:'pointer',width:'100%',fontSize:'0.82rem',fontWeight:filterBG===bg?700:400,transition:'all 0.15s'}}>
...
export default function BloodBankPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (storedUser.id) {
          const res = await axios.get(
            `${process.env.NEXT_PUBLIC_API_URL}/users/${storedUser.id}`,
            {headers:{Authorization:`Bearer ${tkn}`}}
          );
          setUser(res.data);
        } else {
    ...
    initUser();
    Promise.all([fetchRequests(tkn), fetchDonors(tkn), fetchMyDonorProfile(tkn)])
  ...
  const handleDonorSubmit = async (e) => {
    e.preventDefault();
    if (!user?.bloodGroup) {
      alert('Please add your blood group in your profile first!');
      return;
    }
    setSubmitting(true);
    try {
      const submitData = { ...donorForm, bloodGroup: user.bloodGroup };
      const res = await axios.post(
    ...
    <div className="page-bg">
      ...
      <div className="main-with-sidebar">
        <div className="center-wrap" style={{flex:1,paddingTop:'2rem',paddingBottom:'3rem'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.5rem',flexWrap:'wrap',gap:'0.75rem'}}>
            <div>
              <h2 style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:'1.6rem',color:'white',marginBottom:'0.3rem',display:'flex',alignItems:'center',gap:'0.5rem'}}>
                <Droplets size={26} color="#ef4444"/> Blood Bank
              </h2>
              <p style={{color:'var(--text2)',fontSize:'0.875rem'}}>Request blood or register as a donor for IIUC community</p>
            </div>
            <div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap'}}>
              <button onClick={()=>setShowRequestForm(true)} className="btn-danger"
                ...
                <Plus size={15}/> Need Blood
              </button>
              <button onClick={()=>setShowDonorForm(true)} className="btn-primary"
                style={{padding:'0.55rem 1.1rem',fontSize:'0.875rem',display:'flex',alignItems:'center',gap:'0.4rem'}}>
                {myDonorProfile ? <><Edit size={15}/> Update Donor Info</> : <><Heart size={15}/> Become a Donor</>}
              </button>
            </div>
          </div>
          ...
          <div style={{display:'flex',gap:'0.35rem',marginBottom:'1.25rem',borderBottom:'1px solid rgba(34,197,94,0.1)',paddingBottom:'0.5rem'}}>
            {[
              ...
              <button key={t.key} onClick={()=>setTab(t.key)} style={{
                ...
                color:tab===t.key?t.color:'var(--text2)',display:'flex',alignItems:'center',gap:'0.4rem',transition:'all 0.2s'
              }}>
                {t.label}
                <span style={{background:tab===t.key?`${t.color}33`:'rgba(255,255,255,0.08)',color:tab===t.key?t.color:'var(--text3)',borderRadius:'999px',padding:'0 6px',fontSize:'0.72rem',fontWeight:700}}>
                  {t.count}
                </span>
              </button>
            ))}
          ...
          <div style={{display:'flex',alignItems:'center',gap:'0.5rem',marginBottom:'1.25rem',flexWrap:'wrap'}}>
            <div ref={filterRef} style={{position:'relative'}}>
              <button onClick={()=>setShowFilter(!showFilter)} style={{
                display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.45rem 0.875rem',borderRadius:'9px',
                ...
                color:filterBG!=='all'?bgColor(filterBG):'var(--text2)',cursor:'pointer',fontSize:'0.82rem',fontWeight:600,transition:'all 0.2s'
              }}>
                <Filter size={14}/>
                {filterBG==='all'?'Filter':filterBG}
                {filterBG!=='all' && (
                  <span onClick={e=>{e.stopPropagation();handleBGFilter('all');}} style={{marginLeft:'0.15rem',color:'var(--text3)',lineHeight:1,fontSize:'1rem'}}>×</span>
                )}
              </button>
            ...
            {tab==='donors' && (
              <>
                <button onClick={()=>setShowLocationSearch(!showLocationSearch)} style={{
                  ...
                  color:filterLocation?'#22c55e':'var(--text2)',cursor:'pointer',fontSize:'0.82rem',fontWeight:500
                }}>
                  <Search size={14}/>
                  {filterLocation||'Location'}
                  {filterLocation && (
                    <span onClick={e=>{e.stopPropagation();setFilterLocation('');fetchDonors(token,filterBG,'');}} style={{marginLeft:'0.15rem',color:'var(--text3)',fontSize:'1rem'}}>×</span>
                  )}
                </button>
                {showLocationSearch && (
                  <input type="text" placeholder="Type location..." value={filterLocation}
                    ...
                    className="input-field" style={{fontSize:'0.8rem',maxWidth:'180px',borderRadius:'999px'}} autoFocus
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'0.75rem'}}>
              {requests.length===0 ? (
                <div style={{textAlign:'center',padding:'4rem 1rem'}}>
                  <Droplets size={48} style={{color:'rgba(255,255,255,0.08)',margin:'0 auto 0.75rem',display:'block'}}/>
                ...
                <div key={req.id} className="glass-card pulse-hover" style={{
                  padding:'1.1rem',
                  border: req.status==='fulfilled'?'1px solid rgba(34,197,94,0.2)':req.urgent?'1px solid rgba(239,68,68,0.35)':undefined,
                  opacity:req.status==='fulfilled'?0.8:1
                }}>
                  ...
                  <div style={{display:'flex',alignItems:'flex-start',gap:'0.875rem'}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:'0.4rem'}}>
                        <div>
                          <p style={{color:'white',fontWeight:700,fontSize:'0.95rem',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{req.patientName}</p>
                          <div style={{display:'flex',flexDirection:'column',gap:'0.25rem',marginTop:'0.35rem'}}>
                            <p style={{color:'var(--text2)',fontSize:'0.8rem'}}>🏥 {req.hospital}</p>
                            <p style={{color:'var(--text2)',fontSize:'0.8rem',display:'flex',alignItems:'center',gap:'0.35rem'}}>
                              <MapPin size={12} color="#22c55e"/> {req.location}
                            </p>
                            <p style={{color:'var(--text2)',fontSize:'0.8rem'}}>📞 {req.contact}</p>
                          </div>
                          {req.details && (
                            <p style={{color:'var(--text3)',fontSize:'0.78rem',marginTop:'0.35rem',fontStyle:'italic'}}>"{req.details}"</p>
                          )}
                        </div>
                        <div style={{display:'flex',flexDirection:'column',gap:'0.4rem',alignItems:'flex-end'}}>
                          <Link href={`/users/${req.user?.id}`} style={{textDecoration:'none',display:'flex',alignItems:'center',gap:'0.5rem'}}>
                            <span style={{color:'var(--text3)',fontSize:'0.7rem'}}>{req.user?.name}</span>
                            <Avatar user={req.user} size={22} radius="50%"/>
                          </Link>
                          <span style={{color:'var(--text3)',fontSize:'0.68rem'}}>{new Date(req.createdAt).toLocaleDateString()}</span>
                          {req.userId===user?.id && (
                      ...
                      {req.userId!==user?.id && req.status!=='fulfilled' && (
                        <button onClick={()=>router.push(`/messages?userId=${req.user?.id}&userName=${encodeURIComponent(req.user?.name)}`)}
                          className="btn-outline" style={{marginTop:'0.65rem',padding:'0.4rem 0.875rem',fontSize:'0.78rem',display:'flex',alignItems:'center',gap:'0.35rem',width:'fit-content'}}>
          ...
          {tab==='donors' && (
            <>
              {myDonorProfile && (
                <div className="glass-card" style={{marginBottom:'1rem',padding:'1.1rem',border:'1px solid rgba(34,197,94,0.25)',background:'rgba(34,197,94,0.04)'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:'0.875rem',flexWrap:'wrap'}}>
                    ...
                    <div style={{flex:1}}>
                      <p style={{color:'var(--text2)',fontSize:'0.82rem',display:'flex',alignItems:'center',gap:'0.35rem'}}>
                        <MapPin size={12} color="#22c55e"/> {myDonorProfile.location}
                      </p>
                      {myDonorProfile.lastDonationDate && (
                        <p style={{color:'var(--text3)',fontSize:'0.78rem',display:'flex',alignItems:'center',gap:'0.35rem',marginTop:'0.2rem'}}>
                      ...
                      <span style={{
                        ...
                        padding:'1px 8px',borderRadius:'999px',fontSize:'0.7rem',fontWeight:600,display:'inline-block',marginTop:'0.25rem'
                      }}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'0.875rem'}}>
                {donors.length===0 ? (
                  <div style={{textAlign:'center',padding:'3rem',gridColumn:'1/-1'}}>
                    <Heart size={48} style={{color:'rgba(255,255,255,0.08)',margin:'0 auto 0.75rem',display:'block'}}/>
                    <p style={{color:'var(--text2)',marginBottom:'1rem'}}>
                  ...
                  <div key={donor.id} className="glass-card pulse-hover" style={{padding:'1rem',display:'flex',flexDirection:'column',gap:'0.65rem'}}>
                    <Link href={`/users/${donor.userId}`} style={{textDecoration:'none',display:'flex',alignItems:'center',gap:'0.65rem'}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <p style={{color:'white',fontWeight:600,fontSize:'0.875rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:"'Plus Jakarta Sans',sans-serif",transition:'color 0.2s'}}
                          onMouseEnter={e=>e.currentTarget.style.color='#22c55e'}
                          onMouseLeave={e=>e.currentTarget.style.color='white'}>
                          {donor.user?.name}
                        </p>
                        <p style={{color:'var(--text3)',fontSize:'0.72rem'}}>{donor.user?.department} • Batch {donor.user?.batch}</p>
                      </div>
                      <BGBadge bg={donor.bloodGroup}/>
                    </Link>

                    <div style={{display:'flex',flexDirection:'column',gap:'0.3rem'}}>
                      <p style={{color:'var(--text2)',fontSize:'0.78rem',display:'flex',alignItems:'center',gap:'0.4rem'}}>
                        <MapPin size={12} color="#22c55e"/> {donor.location}
                      </p>
                      {donor.lastDonationDate && (
                        <p style={{color:'var(--text3)',fontSize:'0.75rem',display:'flex',alignItems:'center',gap:'0.4rem'}}>
                      ...
                      <span style={{
                        ...
                        padding:'1px 8px',borderRadius:'999px',fontSize:'0.7rem',fontWeight:600,display:'inline-block',width:'fit-content'
                      }}>
                    ...
                    {donor.userId!==user?.id ? (
                      <div style={{display:'flex',gap:'0.4rem'}}>
                        <button onClick={()=>window.open(`https://wa.me/${formatWhatsApp(donor.whatsapp)}?text=${encodeURIComponent(`Hi ${donor.user?.name}, I need ${donor.bloodGroup} blood. Can you help?`)}`,'_blank')}
                          style={{flex:1,padding:'0.5rem',borderRadius:'9px',border:'none',cursor:'pointer',background:'linear-gradient(135deg,#25d366,#128c7e)',color:'white',fontWeight:600,fontSize:'0.78rem',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.35rem',transition:'opacity 0.2s'}}
                          onMouseEnter={e=>e.currentTarget.style.opacity='0.85'}
                          onMouseLeave={e=>e.currentTarget.style.opacity='1'}>
                          💬 WhatsApp
                        </button>
                        <button onClick={()=>setDirectMsgDonor(donor)}
                          style={{flex:1,padding:'0.5rem',borderRadius:'9px',cursor:'pointer',background:'rgba(34,197,94,0.1)',border:'1px solid rgba(34,197,94,0.3)',color:'#22c55e',fontWeight:600,fontSize:'0.78rem',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.35rem',transition:'all 0.2s'}}
                      ...
                      <p style={{color:'var(--text3)',fontSize:'0.72rem',textAlign:'center'}}>This is your profile</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      {showRequestForm && (
        <div className="modal-overlay" onClick={()=>setShowRequestForm(false)}>
          <div className="glass-card modal-content" style={{width:'100%',maxWidth:'500px',padding:'1.5rem',maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
              <h3 style={{color:'#ef4444',fontWeight:700,fontSize:'1rem',fontFamily:"'Plus Jakarta Sans',sans-serif",display:'flex',alignItems:'center',gap:'0.4rem'}}>
                <Droplets size={16}/> Blood Request
              </h3>
              <button onClick={()=>setShowRequestForm(false)} style={{background:'transparent',border:'none',color:'var(--text3)',cursor:'pointer'}}><X size={18}/></button>
            </div>
            <form onSubmit={handleRequestSubmit} style={{display:'flex',flexDirection:'column',gap:'0.75rem'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.65rem'}}>
                <div>
                  <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Blood Group *</label>
                ...
                <div>
                  <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Patient Name *</label>
              ...
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Hospital *</label>
              ...
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Location *</label>
              ...
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Contact Number *</label>
              ...
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Additional Details</label>
      ...
      {/* Donor Form Modal */}
      {showDonorForm && (
        <div className="modal-overlay" onClick={()=>setShowDonorForm(false)}>
          <div className="glass-card modal-content" style={{width:'100%',maxWidth:'460px',padding:'1.5rem',maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
              <h3 style={{color:'#22c55e',fontWeight:700,fontSize:'1rem',fontFamily:"'Plus Jakarta Sans',sans-serif",display:'flex',alignItems:'center',gap:'0.4rem'}}>
                <Heart size={16}/> {myDonorProfile?'Update Donor Profile':'Register as Donor'}
              </h3>
              <button onClick={()=>setShowDonorForm(false)} style={{background:'transparent',border:'none',color:'var(--text3)',cursor:'pointer'}}><X size={18}/></button>
            </div>
            <form onSubmit={handleDonorSubmit} style={{display:'flex',flexDirection:'column',gap:'0.75rem'}}>
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>
                  Blood Group <span style={{color:'var(--text3)',fontWeight:400}}>(from your profile)</span>
                </label>
                {user?.bloodGroup ? (
                  <div style={{display:'flex',alignItems:'center',gap:'0.75rem',padding:'0.6rem 0.875rem',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'10px'}}>
                    <BGBadge bg={user.bloodGroup} size="large"/>
                    <span style={{color:'var(--text3)',fontSize:'0.75rem'}}>🔒 To change, update your profile</span>
                  </div>
                ) : (
                  <div style={{padding:'0.75rem',background:'rgba(239,68,68,0.06)',borderRadius:'10px',border:'1px solid rgba(239,68,68,0.2)'}}>
                    <p style={{color:'#f87171',fontSize:'0.82rem',marginBottom:'0.4rem'}}>⚠️ No blood group in your profile.</p>
                    <button type="button" onClick={()=>{setShowDonorForm(false);router.push('/profile');}}
                      style={{background:'rgba(34,197,94,0.1)',border:'1px solid rgba(34,197,94,0.3)',color:'#22c55e',borderRadius:'8px',padding:'0.4rem 0.875rem',cursor:'pointer',fontSize:'0.82rem',fontWeight:600}}>
                      → Go to Profile to add Blood Group
                    </button>
                  </div>
                )}
              </div>
              ...
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>WhatsApp Number *</label>
                ...
                <p style={{color:'var(--text3)',fontSize:'0.7rem',marginTop:'0.2rem'}}>🔒 Hidden from public. WhatsApp button redirects to your inbox directly.</p>
              </div>
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Last Donation Date</label>
                ...
                <p style={{color:'var(--text3)',fontSize:'0.7rem',marginTop:'0.2rem'}}>Update this after every donation</p>
              </div>
              <div>
                <label style={{color:'var(--text3)',fontSize:'0.72rem',fontWeight:600,display:'block',marginBottom:'0.4rem',textTransform:'uppercase',letterSpacing:'0.04em'}}>Availability *</label>
                <div style={{display:'flex',gap:'0.5rem'}}>
                  {[{val:true,label:'✅ Available',color:'#22c55e'},{val:false,label:'❌ Not Available',color:'#ef4444'}].map(opt=>(
                    <button key={String(opt.val)} type="button" onClick={()=>setDonorForm(p=>({...p,available:opt.val}))}
                      style={{flex:1,padding:'0.5rem',borderRadius:'9px',cursor:'pointer',fontSize:'0.8rem',fontWeight:600,
                        ...
                        color:donorForm.available===opt.val?opt.color:'var(--text2)',transition:'all 0.2s'}}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              <button type="submit" disabled={submitting||!user?.bloodGroup} className="btn-primary"
                style={{width:'100%',padding:'0.65rem',fontSize:'0.875rem',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.4rem',
                  opacity:!user?.bloodGroup?0.5:1,cursor:!user?.bloodGroup?'not-allowed':'pointer'}}>
                <Heart size={16}/> {submitting?'Saving...':myDonorProfile?'Update Profile':'Register as Donor'}
              </button>
              {!user?.bloodGroup && (
                <p style={{color:'#f87171',fontSize:'0.75rem',textAlign:'center'}}>⚠️ Add your blood group in profile first</p>
              )}
            </form>
          </div>
        </div>
